...
SAVE_MODEL_DIR = '/data2/torch_model_7th_small_spins_POWER12_B5000_batch12/'
...
# 모델을 만들 AB 범위 _ only for small spins
A_start = -60000
A_end = 60000
B_start = 2000
B_end = 17000
...
A_model_step = 3  # model step
B_model_step = 3  # model step
A = np.arange(0, len(A_range_list), A_model_step)
B = np.arange(0, len(B_range_list), B_model_step)
...
for count, [i, j] in enumerate(AB_indices):
    A_test = A_range_list[i:i+A_model_step]
    B_test = B_range_list[j:j+B_model_step] 
...
train_data_batch = 4096 * 12
valid_data_batch = 2048
batch_size = 4096
learning_rate = 1e-4
epochs = 100
...
#         POWER=8
    ...
    print("Model Index: ", idx_model, "total index length:", len(total_filtered_idx), "PRE_PRO:", PRE_PROCESSING)
    ...
    print("========== The AB_range of the model ==========")
    for key in AB_dic: print(AB_dic[key])
    print("\n(X_train, Y_train, X_val, Y_val) : ", X_train_arr.shape, Y_train_arr.shape, X_valid_arr.shape, Y_valid_arr.shape)
    ...
    print("Prediction..")
    denoise_temp = 2*denoise_data[:12000] - 1 
    denoise_temp /= slope
    exp_data_temp = 2*exp_data[:12000] - 1
    exp_data_unslope = exp_data_temp / slope
    
    if PRE_PROCESSING == True:
        new_denoise = pre_processing(denoise_temp, power=POWER)
        new_exp_data = pre_processing(exp_data_temp, power=POWER)
        exp_data_unslope = pre_processing(exp_data_unslope, power=POWER)
        
    denoise_temp_tensor = torch.FloatTensor(denoise_temp[total_filtered_idx].reshape(1, -1)).cuda()
    exp_data_temp_tensor = torch.FloatTensor(exp_data_temp[total_filtered_idx].reshape(1, -1)).cuda()
    exp_data_unslope_tensor = torch.FloatTensor(exp_data_unslope[total_filtered_idx].reshape(1, -1)).cuda()

    pred_denoise = model(denoise_temp_tensor)
    pred_exp = model(exp_data_temp_tensor)
    ...
    print("========== Training Done. The AB_range of the model ==========")
    for key in AB_dic: print(AB_dic[key])