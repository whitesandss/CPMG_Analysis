...
SAVE_MODEL_DIR = '/data2/torch_model_5th_small_9nodes/'
POOL_PROCESS = 15
CUDA_DEVICE = 0
...
n_of_spins = 28
n_of_small_spins = 13
n_of_samples_per_target = 160000
...
# Sample Number
N_SAMPLES_PER_TARGET = batch_size * 12
...
#     if (idx_model < 50) | (idx_model > 100):
    ...
    Y_train_arr = np.zeros((AB_lists.shape[0], X_train_arr.shape[1], AB_lists.shape[0]-1))
    X_valid_arr = np.zeros((AB_lists.shape[0], valid_data_batch, total_filtered_idx.shape[0]))
    Y_valid_arr = np.zeros((AB_lists.shape[0], X_valid_arr.shape[1], AB_lists.shape[0]-1))
    ...
    new_denoise /= slope
    exp_data = 2*exp_data[:12000] - 1
    exp_data_unslope = exp_data / slope
    ...
    exp_data_tensor = torch.FloatTensor(exp_data[total_filtered_idx].reshape(1, -1)).cuda()
